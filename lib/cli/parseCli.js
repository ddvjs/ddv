...
c.__connect = function(callback){
	...
	q.end(function onEnd(state, res){
		if (q&&callback&&b.type(callback, 'function')) {
			callback(state?null:res);
		}
		q = callback = void 0 ;
	});
	//检测是否有存在的api
	q.push(function checkApi(next, success){
	...
	//检测是否有存在的api
	q.push(true, function connectDaemonRpcApi(next, success){
		try{
	...
	//检测是否有存在的api
	q.push(true, function checkStartDaemon(next, success, fail){
		if (startDaemon&&b.type(startDaemon, 'function')) {
			startDaemon((err)=>{
				if (err) {
					fail(err);
				}else{
	...
	//检测是否有存在的api
	q.push(true, function connectDaemonRpcApi2(next, success, fail){
		try{
			c.api = new Api((err, pid)=>{
				if (err) {
					fail(err);
...
c.__api = function(is_auto_disconnect, cmd_name, args, callback){
	if (!b.type(is_auto_disconnect,'boolean')) {
		callback = args;
		args = cmd_name;
		cmd_name = is_auto_disconnect;
	...
	q.end(function onEnd(state, res){
		...
		if (is_auto_disconnect===true) {
			try{
				c.api.disconnect();
			}catch(e){}
		}
		q = callback = is_auto_disconnect = void 0 ;
	});
	//检测是否有存在的api
	q.push(function checkApi(next, success, fail){
		if (c.api&&c.api.state===true) {
			process.nextTick(function() {
				next();
			});
	...
	//试图运行api
	q.push(true, function callApi(next, success, fail){
		if (c.api[cmd_name]&&b.type(c.api[cmd_name], 'function')) {
			c.api[cmd_name]((args||{}), (err, res)=>{
				if (err) {
					fail(err);
				}else{
					success(res);
			...
			let err = new Error('find not cmd '+cmd_name+'!', 'find not cmd '+cmd_name+'!');
