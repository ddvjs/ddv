'use strict'
// 调试模块模块
const debug = require('debug')('cli/rpc/index')
// ddv 地址模块
...
Object.assign(commander, {
  ...
  rpcBySiteIdName (type) {
    ...
    return rpc(type, args).then(res => {
      log.tip(' OK ', `cli.command.${type.toUpperCase()}.${isServer ? 'server' : 'stie'}.success`)
      isServer = type = void 0
      return commander.rpcSiteLists()
    }).catch(err => {
      log.tip(' ERR ', `cli.command.${type.toUpperCase()}.${isServer ? 'server' : 'stie'}.fail`)
      isServer = type = void 0
      log.error(err)
    })
  }
