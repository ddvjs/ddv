...
master.on('server::listening', function masterError (info) {
  log.info('master.listen.ing', info && info.type_port_ip)
})
master.on('server::close', function masterError (info) {
  log.info('master.listen.close', info && info.type_port_ip)
})
master.on('server::error', function masterError (e) {
  log.error(
    (e.code === 'EACCES' && e.message.indexOf('listen EACCES') > -1) ? 'master.server.eacces' : 'master.server.error'
  ).then(() => {
    return log.error(e)
  })
  .then(() => {
