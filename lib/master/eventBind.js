...
master.onDaemonCall('getSiteInfoOne', function processInfoCb ({siteId}) {
  var getChildren = []
  var wids = (sys && sys.siteIdToWorkerInfo && sys.siteIdToWorkerInfo[siteId] && sys.siteIdToWorkerInfo[siteId].wids) || []
  Array.isArray(wids) && wids.forEach(wid => {
    getChildren.push(
      master.callWorker(wid, 'processInfo').then(({data}) => {
        // 名字
        data.name = 'w:' + wid
        // 状态
  ...
  return Promise.all(getChildren).then(children => ({children}))
})
