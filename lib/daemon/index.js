'use strict'
const debug = require('debug')('lib/daemon/index')
// ddv路径模块
...
// 事件绑定
const eventBind = require(ddvPath('lib/daemon/eventBind'))
// api接口
const daemonApi = require(ddvPath('lib/daemon/api.daemon'))
// 是否存在守护进程
...
.then(() => {
  ...
  if (isHasParentProcess()) {
    ...
    return (isRepeatDaemon ? log.info('DAEMON_RUN_RES_DAEMON_REPEAT') : Promise.resolve()).then(() => {
      return log.tip(' OK ', 'DAEMON_RUN_RES_DAEMON_SUCCSS')
    })
  }
}, e => {
  // 返回启动失败
  if (isHasParentProcess()) {
    ...
    return (isRepeatDaemon ? log.info('DAEMON_RUN_RES_DAEMON_REPEAT') : Promise.resolve()).then(() => {
      log.tip('FAIL', 'DAEMON_RUN_RES_DAEMON_FAIL').then(() => {
        process.exit(1)
      })
    })
  }
