'use strict'
const debug = require('debug')('server/command')
// ddv路径模块
...
const commandEventBind = function commandEventBind (WebSocket) {
    ...
    getDaemon().then(daemon => {
      ...
      return Promise.all([
        new Promise((resolve, reject) => {
          debug('rpcapi::command::lists -- getMaster')
          // 查询master状态
  ...
  WebSocket.on(['rpcapi', 'command', '/v1_0/command/remove'], (headers) => {
    ...
    if (argv) {
      siteManage.remove.then(() => callback && callback(null, null), e => callback && callback(e, null))
    } else {
      let err = new Error('The command line argument can not be null')
      err.error_id = 'COMMAND_ARGV_ERROR'
      callback(err)
  ...
  WebSocket.on(['rpcapi', 'command', '/v1_0/command/kill'], (headers) => {
    ...
    Array.isArray(pids) && pids.forEach(processId => {
      ...
      if (isKillPid === false) {
        return new Promise((resolve, reject) => {
          daemon.kill(function killCb (pids) {
            Array.isArray(pids) && pids.forEach(pidt => {
              try {
                process.kill(pidt[0])
                pidt.state = true
              } catch (e) {}
