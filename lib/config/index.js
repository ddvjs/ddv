  ...
  // 并发队列
    ...
    return Promise.all([
      ...
      Promise.resolve().then(function getBaseInfo () {
        ...
        // 运行的环境目录
        fn.execPath = (process.platform !== 'darwin' ? ddvPath.dirname(process.execPath) : process.env.PATH) || config.execPath
        // 获取 用户home目录
...
fn.getSaveConfig = function getSaveConfig (saveConfig) {
  ...
  return new Promise(function (resolve, reject) {
    for (let key in saveConfig) {
      ...
      if (saveConfig[key] && (!util.type(saveConfig[key], 'string')) && (!util.type(saveConfig[key], 'number'))) {
        if (!c[key]) {
