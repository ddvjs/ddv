...
API.__init = function(address, getApiCallback){
	...
	this.on('request::response',(rowraw)=>{
		...
		if(!(rowraw.headers&&(request_id = rowraw.headers.request_id))){
			return;
		}
		if (this.processWs&&(o = this.processWs[request_id])) {
			...
			delete this.processWs[request_id];
		}
...
//遍历支持一下命令行
b.each('add remove lists start restart stop kill'.split(' '),function(index, name) {
