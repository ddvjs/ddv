...
const Api = module.exports = class DdvApi extends domain.Domain {
  ...
  constructor (o) {
    ...
    // 基本数据初始化
    this._baseInit(o)
    debug('new DdvApi address', this.address)
    debug('new DdvApi rpcTimeout', this.rpcTimeout)
    // 返回实例化对象或者开始连接
  ...
  setRpcTimeout (rpcTimeout) {
    // 设置超时时间
    rpcTimeout || (rpcTimeout = rpcTimeout || this.rpcTimeout || API.rpcTimeout)
  }
  // 连接
  connect () {
    debug('connect DdvApi address', this.address)
    debug('connect DdvApi rpcTimeout', this.rpcTimeout)
...
API.__init = function (address, getApiCallback) {
  ...
  if (!address) {
    try {
      ...
      if (getApiCallback && util.isFunction(getApiCallback)) {
        getApiCallback(new Error('address error!'))
      }
    }
  }
  if (!util.type(address, 'string')) {
    getApiCallback(new Error('address error!'))
    return
  }
  try {
...
// 遍历支持一下命令行
util.each('add remove lists start restart stop kill guid'.split(' '), function (index, name) {
