...
Satan.checkConfig = function checkConfig (callback) {
  return new Promise(function checkConfig (resolve, reject) {
    // 检测试验有config配置信息
    if (!config) {
      try {
        Satan.getConfig()
...
Satan.pingDaemon = function daemonPing (callback) {
  return Satan.checkConfig().then(function tryNewApi () {
    return new Promise(function tryNewApi (resolve, reject) {
      try {
        // 检测后台线程
...
Satan.start = function daemonStart () {
  return Satan.pingDaemon().catch(function () {
...
Satan.launchDaemon = function launchDaemon () {
  return Satan.checkConfig().then(function baseConfig () {
    // 是否开启调试模式
  ...
  }).then(function pingDaemon () {
    // winNt内核
    if (config.platform === 'win32' || config.platform === 'win64') {
      return fn.launchDaemonByWin()
    } else {
    // Unix内核
      return fn.launchDaemonByUnix()
    }
  })
}
