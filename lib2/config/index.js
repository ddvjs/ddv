...
cjbbase.extend(init, {
  ...
  baseInfo: function baseInfo () {
    ...
    // ddv入口文件
    config.ddvMainFile = process.env.DDV_MAIN_PATH || process.mainModule.filename || config.ddvMainFile
    // 不保存进入配置文件目录
    ...
    if (!fs.existsSync(dirpath)) {
      ...
      dirpath.split(path.sep).forEach(function (dirname) {
        ...
        try {
          ...
          isExists = Boolean(stat.isDirectory())
        } catch (e) {}
        if (!isExists) {
  ...
  saveConfig: function saveConfig () {
    var key
    var writeConfig = Object.create(null)
    for (key in config) {
